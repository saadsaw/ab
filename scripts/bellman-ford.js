...
async function start() {
    ...
    let found = false;
    if (sn && gn) {
        let bf = cy.elements().bellmanFord({ root: '#' + sn });
        ...
        // Animation Code
        for (let i = 0; i < bf.pathTo('#' + gn).length; i++) {
            bf.pathTo('#' + gn)[i].addClass('highlighted');
            if (bf.pathTo('#' + gn)[i].data('id') == gn) {
                bf.pathTo('#' + gn)[i].addClass('goalNode');
                break;
            }
