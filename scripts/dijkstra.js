...
async function start() {
    ...
    let found = false;
    if (sn && gn) {
        let dijkstra = cy.elements().dijkstra({ root: '#' + sn });
        ...
        // Animation Code
        for (let i = 0; i < dijkstra.pathTo('#' + gn).length; i++) {
            dijkstra.pathTo('#' + gn)[i].addClass('highlighted');
            if (dijkstra.pathTo('#' + gn)[i].data('id') == gn) {
                dijkstra.pathTo('#' + gn)[i].addClass('goalNode');
                break;
            }
